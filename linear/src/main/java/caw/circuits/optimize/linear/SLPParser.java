...
public class SLPParser
{
	...
	public MatchResult find_matching_element(String id, ArrayList<SLPElement> inputs, ArrayList<SLPElement> vars, ArrayList<SLPElement> outputs)
	{
	...
	public static void main(String[] args)
	{
		...
		try
		{
			...
			int numGates = Integer.parseInt(data[0]);

			// Read SLP inputs
			ArrayList<SLPElement> inputs = new ArrayList<SLPElement>();
			line = in.readLine();
			data = line.split(" ");
			...
			// Read SLP outputs
			ArrayList<SLPElement> outputs = new ArrayList<SLPElement>();
			line = in.readLine();
			data = line.split(" ");
			...
			ArrayList<SLPLine> lines = new ArrayList<SLPLine>();
			ArrayList<SLPElement> vars = new ArrayList<SLPElement>();
			for (int i = 0; i < numGates; i++)
			{
				...
				MatchResult outputResult = parser.find_matching_element(output, inputs, vars, outputs);
				if (outputResult.newVariable)
				{
					vars.add(outputResult.element);
				}
				slpOutput = outputResult.element;
				MatchResult op1Results = parser.find_matching_element(o1, inputs, vars, outputs);
				if (op1Results.newVariable)
				{
					vars.add(op1Results.element);
				}
				slpO1 = op1Results.element;
				MatchResult op2Results = parser.find_matching_element(o2, inputs, vars, outputs);
